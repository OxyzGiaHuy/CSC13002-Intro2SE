...
import { openGoogleMapsDirections, openTrailRoute } from '../utils/mapUtils';
...
const TrailDetail: React.FC<TrailDetailProps> = ({ trailId, onBack, trails, onToggleFavorite, onSelectMap }) => {
    ...
    const getCoords = () => {
        // 1. Lấy giá trị mặc định từ các trường phẳng (nếu backend trả về số)
        let sLat = trail.start_lat;
        let sLng = trail.start_lng;
        let eLat = trail.end_lat;
        let eLng = trail.end_lng;

        // 2. Kiểm tra nếu backend trả về GeoJSON Object (start_point), ưu tiên lấy từ đây
        // GeoJSON Format: [Longitude, Latitude] -> [0] là Kinh độ, [1] là Vĩ độ
        if (trail.start_point && trail.start_point.coordinates) {
            sLng = trail.start_point.coordinates[0];
            sLat = trail.start_point.coordinates[1];
        }

        if (trail.end_point && trail.end_point.coordinates) {
            eLng = trail.end_point.coordinates[0];
            eLat = trail.end_point.coordinates[1];
        }

        // 3. Ép kiểu về Number để đảm bảo không bị lỗi NaN hoặc String
        return {
    ...
    // Điều kiện: Tọa độ phải là số (không phải NaN) và khác 0
    const hasRoute = !isNaN(coords.startLat) && coords.startLat !== 0 &&
                     !isNaN(coords.startLng) && coords.startLng !== 0 &&
                     !isNaN(coords.endLat) && coords.endLat !== 0 &&
                     !isNaN(coords.endLng) && coords.endLng !== 0;

    // Log kiểm tra lần cuối (Bạn có thể comment lại khi đã chạy ổn)
    console.log(`[Trail Check] ID: ${trail.id} | Has Route: ${hasRoute}`, coords);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                            ...
                            {/* NÚT CHUYỂN HƯỚNG BẢN ĐỒ */}
                            <button
                                onClick={() => {
                                    if (hasRoute) {
                                        console.log('Opening Trail Route:', coords);
                                        openTrailRoute(
                                            { lat: coords.startLat, lng: coords.startLng },
                                            { lat: coords.endLat, lng: coords.endLng }
                                        );
                                    } else {
                                        console.log('Fallback to simple directions');
                                        // Fallback an toàn nếu không có lat/lng ở root
                                        const fallbackLat = trail.lat || (trail.start_point?.coordinates ? trail.start_point.coordinates[1] : 0);
                                        const fallbackLng = trail.lng || (trail.start_point?.coordinates ? trail.start_point.coordinates[0] : 0);
                                ...
                                className="bg-blue-600 hover:bg-blue-700 text-white p-6 rounded-2xl shadow-xl shadow-blue-900/20 flex flex-col items-center justify-center transition-all transform hover:scale-[1.02] active:scale-95"
                                title={hasRoute ? "Xem lộ trình leo (Start -> End)" : "Chỉ đường đến đây"}
                            >
                                <Navigation className="w-6 h-6 mb-2" />
                                <span className="font-black text-xs uppercase tracking-widest">
                                    {hasRoute ? 'View Trail Route' : 'Get Directions'}
                                </span>
                            </button>
                            <button
