...
const TrailDetail: React.FC<TrailDetailProps> = ({ trailId, onBack, trails, onToggleFavorite, onSelectMap }) => {
    const trail = trails.find(t => (t.id === trailId || String(t.id) === String(trailId)));
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
                <div className="relative">
                    <img src={imageSrc} alt={trail.name} className="w-full h-64 md:h-96 object-cover" />
                ...
                <div className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mb-6 border-b pb-6">
                        ...
                        <div><p className="text-sm text-gray-500">Length</p><p className="font-bold text-lg">{lengthKm ?? '-'} km</p></div>
                        <div><p className="text-sm text-gray-500">Duration</p><p className="font-bold text-lg">{durationHr ?? '-'} hr</p></div>
                        <div><p className="text-sm text-gray-500">Rating</p><p className="font-bold text-lg">{rating} ★</p></div>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {scenery.map((s: string) => <span key={s} className="bg-light-tan text-earth-brown px-3 py-1 rounded-full text-sm">{s}</span>)}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <div className="space-y-4">
                                {reviews.map((r: any) => (
                                    <div key={r.username || r.id || Math.random()} className="flex gap-3">
